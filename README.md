...
Набор данных скачивается методом `download()` и кэшируется локально, повторный вызов ничего не качает; до загрузки `get_texts()` и `get_records()` поднимают `DatasetNotFoundError` (наследник `OSError`) с подсказкой. Корпус поэзии и классическая литература загружаются с первоисточников по закреплённым коммитам с проверкой SHA-256.
