...
Функция возвращает оси `Axes` с визуализацией; фигура доступна как `ax.figure`. Цвет квадрата - значение метрики сегмента относительно максимума по всем текстам; сегменты, на которых метрика не определена (`nan` на слишком коротких для нее сегментах), рисуются как нулевые, а не пропадают с графика.
