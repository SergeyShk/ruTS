...
Размер словаря $V$ после каждого слова текста и кривая подгонки $V(N) = K \cdot N^{\beta}$ по [`fit_heaps`](../stats/diversity_stats_funcs.md#heaps_beta) с параметрами в легенде. Для естественных текстов $\beta$ лежит в пределах 0.4-0.6; кривая зависит от порядка слов.
