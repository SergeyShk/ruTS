...
Поиск коннекторов в предложении по словоформам в нижнем регистре без буквы ё (`ruts.utils.find_phrases`): в каждой позиции берется самый длинный («и всё же» не распадается на «и»), найденные не пересекаются. Словарь `load_connectors` читает `ruts/resources/connectors.tsv` - 318 коннекторов с классом из `CONNECTOR_CLASSES` и типом из `CONNECTOR_TYPES`, составленных по классификации Криони, Никина и Филипповой (2008) со сверкой по списку маркеров [Ru-RSTreebank](https://rstreebank.ru/markersFull), базе [Рускон](https://ruslinkers.github.io/) и списку причинных маркеров Тольдовой и др. (2018). Однословные «и», «а», «но» входят в словарь, как в Coh-Metrix и TAACO; «что» и «как» коннекторами не считаются. Результат - список именованных кортежей `Connector(sent, start, end, text, cls, kind)`. Класс `CohesionStats` считает по вхождениям плотность коннекторов на 1000 слов - общую, по классам и по типам.
