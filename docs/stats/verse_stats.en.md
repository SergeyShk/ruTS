...
The meter is not determined (`None`) if after fitting more than a tenth of the polysyllabic word stresses (`VERSE_MAX_DEVIATIONS`) remain on weak positions, or if more than 15% of the dictionary stresses had to be moved to an ictus (`VERSE_MAX_MOVED`) with four or more moves (`VERSE_MIN_MOVED`) - this filters out dolnik, accentual verse, syllabic verse, free verse and prose. The meter is also not fitted when there are fewer than six dictionary stresses of polysyllabic words (`VERSE_MIN_STRESSES`): one or two lines or a prose phrase fit some meter almost always, and such a result is not indicative. A text with letters but without Russian words (Latin script) gives empty statistics - `n_lines` 0, meter `None`, shares `nan`; a text without words (empty, digits and punctuation only) raises `SourceError`. A rhyme scheme is written with letters in order of appearance, unrhymed lines with a hyphen: `ABAB`, `-A-A`. Endings with three or more syllables after the stress count as hyperdactylic.
