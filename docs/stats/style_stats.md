...
Доля отглагольных существительных, плотность производных предлогов, вводных слов и штампов считаются по спискам `COMPOUND_PREPOSITIONS`, `PARENTHETICALS` и `OFFICIALESE_CLICHES` из `ruts.constants` по словоформам без фильтрации (`forms`) независимо от переданного экстрактора.
