...
Точность проверена на наборе [RIFMA](https://github.com/Koziev/Rifma) (5121 строфа с ручной расстановкой ударений и схемами рифмовки, MIT): ударения совпадают с разметкой у 97% слов (у многосложных слов из словаря - 97%, у слов вне словаря - 75%), пары рифмующихся строк находятся с точностью 94% и полнотой 90%, схема строфы целиком совпадает в 78% случаев; метр не определен у 5% строф. Часть поправок `STRESS_CORRECTIONS` собрана по этой же разметке (63 формы, у которых словарь расходится с ней в 80% и более случаев из пяти и больше вхождений), поэтому оценка ударений отчасти внутривыборочная: на отложенной половине набора поправки, собранные по другой половине, добавляют 0.1-0.2 процентного пункта.
