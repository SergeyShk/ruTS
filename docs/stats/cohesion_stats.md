...
Коннекторы ищутся по словоформам в каждом предложении по словарю `ruts/resources/connectors.tsv` (318 единиц, см. [`find_connectors`](cohesion_stats_funcs.md#find_connectors)); свой словарь передается параметром `connectors`. Классы по Криони, Никину и Филипповой (2008):
