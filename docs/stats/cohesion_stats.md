...
Леммы и части речи берутся из первого разбора [pymorphy3](https://github.com/no-plagiarism/pymorphy3), разбор зависимостей не требуется. Пары предложений сравниваются по леммам, а не словоформам: в русском языке повтор слова почти всегда меняет форму (птиц - птицы), и Coh-Metrix для английского считал бы такой повтор только на уровне основ.

| Понятие | Определение |
| :------ | :---------- |
| Существительные | часть речи `NOUN` |
| Местоимения | местоимения-существительные `NPRO` (он, себя, кто, это) и местоименные прилагательные `Apro` (этот, который, мой, весь) |
| Аргументы | существительные и местоимения-существительные `NPRO` |
| Знаменательные слова | существительные, прилагательные, глаголы во всех формах и наречия (`CONTENT_POS`) без местоименных и вводных слов (`STOPWORD_GRAMMEMES`) |
