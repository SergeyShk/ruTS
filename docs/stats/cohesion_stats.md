...
Пары предложений сравниваются по леммам, а не словоформам: в русском языке повтор слова почти всегда меняет форму (птиц - птицы), и Coh-Metrix для английского считал бы такой повтор только на уровне основ. Для объекта `Doc` с разметкой частей речи и лемм леммы, части речи, время и вид берутся из `token.lemma_`, `token.pos_` и `token.morph` модели spaCy, то есть с учетом контекста: в «нож из стали» и «эта сталь» повтор существительного будет найден, а первый разбор pymorphy3 дал бы «стать». Если в `Doc` есть части речи, но нет лемм (пайплайн без лемматизатора), лемма берется из разбора pymorphy3 с той же частью речи (`ruts.utils.lemmatize`), как это делает лемматизатор spaCy для русского языка. Для строки и `Doc` без разметки используется первый разбор [pymorphy3](https://github.com/no-plagiarism/pymorphy3), разбор зависимостей не требуется.

| Понятие | pymorphy3 | Universal Dependencies (`Doc`) |
| :------ | :-------- | :----------------------------- |
| Существительные | `NOUN` | `NOUN`, `PROPN` |
| Местоимения | местоимения-существительные `NPRO` (он, себя, кто) и местоименные прилагательные `Apro` (этот, который, мой, весь), а также слова с леммой из `DEMONSTRATIVE_LEMMAS`, которые pymorphy3 относит к частицам и наречиям (это, столько) | `PRON`, `DET` и те же леммы |
| Аргументы | `NOUN` и `NPRO` | `NOUN`, `PROPN`, `PRON` |
| Знаменательные слова | существительные, прилагательные, глаголы во всех формах и наречия (`CONTENT_POS`) без местоименных и вводных слов (`STOPWORD_GRAMMEMES`) и без указательных | `NOUN`, `PROPN`, `ADJ`, `VERB`, `ADV` (`CONTENT_UD_POS`) без указательных; вспомогательный `AUX` и местоименные наречия (там, тогда), которые модель не отличает от обычных, считаются по правилам UD |
| Указательные | леммы из `DEMONSTRATIVE_LEMMAS`: этот, это, тот, такой, таковой, столько, сей, оный - всегда входят в местоимения, поэтому `p_demonstratives` не превышает `p_pronouns` | те же леммы |
| Время и вид | граммемы `past`, `pres`, `futr` и `perf`, `impf` глагольных форм | признаки `Tense` и `Aspect` |

Показатели повторов лежат в пределах от 0 до 1: для текста из одного предложения они не определены и равны `nan`, как и темпоральная связность текста без глаголов в соседних предложениях. Повторы по всем парам предложений считаются без перебора пар - по битовым маскам вхождений и гистограммам длин предложений (см. [`calc_overlaps`](cohesion_stats_funcs.md#calc_overlaps)), поэтому текст из тысяч предложений обрабатывается за секунды.
