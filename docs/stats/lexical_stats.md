...
Леммы для `Doc` с разметкой частей речи берутся из `token.lemma_` и `token.pos_`, для строки и `Doc` без разметки - из первого разбора pymorphy3. Знаменательные слова определяются как в [`CohesionStats`](cohesion_stats.md).
