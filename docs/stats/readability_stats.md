...
Атрибут `reading_time` оценивает время чтения про себя при скорости 180 слов в минуту (верхняя граница нормы для взрослого по Кузнецову и Хромову, 1991). Другие нормы, в том числе школьные нормы чтения вслух по ФГОС, доступны в справочнике `ruts.constants.READING_SPEED_NORMS` и функции [`calc_reading_time`](readability_stats_funcs.md#calc_reading_time).
