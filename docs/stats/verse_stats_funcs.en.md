...
3.  **Disambiguation.** Monosyllabic prepositions, conjunctions and particles (`VERSE_PROCLITICS`) are unstressed, other monosyllables are stressed only on an ictus; polysyllabic function words (`VERSE_WEAK_WORDS`: или, чтобы, перед, через, после) are stressed if the dictionary stress falls on an ictus. A dictionary stress of a polysyllabic word on a weak position is moved to the ictus if it is the only such word in the line and the word contains exactly one ictus (forms with mobile stress: воды́ - во́ды, реки́ - ре́ки); with two or more such words the line is considered non-metrical and the stresses are kept - on RIFMA a single move agrees with the annotation in 90% of cases, while moves of two or three words per line agree in 66% and 50%, and they forced a meter onto dolnik and accentual verse; a stress by the letter ё and a stress in the anacrusis are never moved. A word without a dictionary stress gets its only ictus; with several ictuses, the stress of the last word of a line is chosen by rhyme with the neighbouring lines, otherwise the last ictus.
...
The accuracy was checked on the [RIFMA](https://github.com/Koziev/Rifma) dataset (5,121 stanzas with manual stress annotation and rhyme schemes, MIT): stresses agree with the annotation for 97% of words (97% for polysyllabic dictionary words, 75% for out-of-dictionary words), pairs of rhyming lines are found with 94% precision and 90% recall, the whole stanza scheme matches in 78% of cases; the meter is undetermined for 5% of stanzas.
