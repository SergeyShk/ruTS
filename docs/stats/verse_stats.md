...
Метр не определяется (`None`), если после подгонки больше десятой части ударений многосложных слов (`VERSE_MAX_DEVIATIONS`) остается на слабых позициях или если перенести на икт пришлось больше 15% словарных ударений (`VERSE_MAX_MOVED`) при четырех и более переносах (`VERSE_MIN_MOVED`) - так отсеиваются дольник, акцентный стих, силлабика, верлибр и проза. Метр не подбирается и при словарных ударениях многосложных слов меньше шести (`VERSE_MIN_STRESSES`): одна-две строки или фраза прозы укладываются в какой-нибудь метр почти всегда, и такой результат не показателен. Текст без русских слов (латиница, цифры) дает пустые статистики - `n_lines` 0, метр `None`, доли `nan`; ошибка `SourceError` поднимается только на тексте без слов. Схема рифмовки записывается буквами по порядку появления, нерифмованные строки - дефисом: `ABAB`, `-A-A`. Окончания с тремя и более заударными слогами считаются гипердактилическими.
