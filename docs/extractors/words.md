...
| `filter_nums` | bool | `False` | Фильтровать числа, в том числе диапазоны, дроби и порядковые числительные (2020-2021, 5.5, 1,5, 3-й) |
...
!!! note "Примечание"
    Фильтры применяются по порядку: знаки препинания, числа, лемматизация, нижний регистр, стоп-слова, длина слова. Стоп-слова сравниваются уже после приведения к нижнему регистру, поэтому при `lowercase=True` список стоп-слов достаточно задать в нижнем регистре. Знаком препинания считается токен, целиком состоящий из знаков и символов, в том числе многосимвольный: `?!`, `!..`, `--`, `…`.
