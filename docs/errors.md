...
!!! info ""
    **ruts.exceptions**, **ruts.__version__**
...
Библиотека ничего не печатает сама: сообщения о загрузке и извлечении наборов данных (`download_file`, `extract_archive`, методы `download()`) отправляются в логгер `ruts` уровня `INFO`, предупреждение о файле, который не является архивом, - уровня `WARNING`. По умолчанию у логгера стоит `NullHandler`, и сообщения не видны; чтобы включить их, настройте логирование в приложении:
...
Версия установленной библиотеки - строка `ruts.__version__`, она читается из метаданных пакета (`importlib.metadata`).
