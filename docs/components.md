...
Модуль для компонента статистик связности текста. Компоненту нужны только границы предложений: подойдет и модель `ru_core_news_sm`, и пустой пайплайн с `sentencizer`. В пайплайне с теггером и лемматизатором леммы и части речи берутся из разметки модели, иначе - из pymorphy3.
