...
Модуль для компонента синтаксических статистик текста. Компонент работает по дереву зависимостей, поэтому в пайплайне должен быть парсер: модели `ru_core_news_sm`, `ru_core_news_md` или `ru_core_news_lg`; в пайплайне без парсера компонент вызывает исключение `SourceError` (наследник `ValueError`).
