...
Модуль для компонента статистик связности текста. Компоненту нужны только границы предложений (модель или `sentencizer`); при наличии разметки частей речи леммы и части речи берутся из нее, иначе - из pymorphy3.
