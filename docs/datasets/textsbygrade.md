...
Модуль для работы с набором данных текстов с метками класса проекта [Plain Russian Language](https://github.com/infoculture/plainrussian) Ивана Бегтина. На этих текстах подбирались коэффициенты формул удобочитаемости для русского языка, которые используются в пресете `plainrussian` класса [`ReadabilityStats`](../stats/readability_stats.md#presets).

Набор содержит 68 текстов, распространяемых под лицензией [CC0 1.0](https://github.com/infoculture/plainrussian/blob/master/LICENSE). Метка - класс школы или год обучения:
...
| Индекс Ганнинга | 0.80 | 4.0 |
| Сводный класс | 0.81 | 3.6 |

Средний сводный класс растет с меткой: 3.0 для 1 класса, 5.8 для 3-4 классов, 6.4 для 5-9, 7.4 для 10-11 и 21.8 для документов с метками 12-17. Большая ошибка на документах ожидаема: формулы класса откалиброваны на школьных текстах, а метка 17 условна.
...
Выполняет извлечение записей (с заголовками) из набора данных. Поля записи: `grade` - уровень сложности, `subject` - наименование текста, `source` - адрес источника (для 13 текстов без опубликованного адреса пустая строка), `text` - текст, `file` - путь к файлу.
