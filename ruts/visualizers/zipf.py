...
def zipf_theory(size: int, num_ranks: int, alpha: float = 1.5) -> list[Line2D]:
    """
    ...
    Вывод:
        plot (list[Line2D]): График теоретического Закона Ципфа
    """
    x = np.arange(1, num_ranks + 1)
    y = x ** (-alpha) / special.zetac(alpha)
    return plt.plot(x, y / max(y) * size, linewidth=2, color="r", label="Теоретический закон")
